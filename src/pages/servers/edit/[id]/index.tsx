...
import { useRoqClient, useServerFindFirst } from 'lib/roq';
...
function ServerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useServerFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ServerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.server.update({
        data: values as RoqTypes.server,
        where: {
          id,
        },
      });
