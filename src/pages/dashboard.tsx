...
import { useRoqPlatformUserProfiles, useInstallationCount, useToolCount, useServerCount } from 'lib/roq';

import { ServerListPage as ShowCaseList } from 'pages/servers';
...
function DashboardPage() {
  ...
  const serverStat = useServerCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Servers',
      result: serverStat?.data,
      isLoading: serverStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
